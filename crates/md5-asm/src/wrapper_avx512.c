...
//
// Bridges between the upstream `md5_state_m128i` layout (one
...
void md5_asm_block_avx512(uint32_t state[4], const uint8_t block[64]) {
    md5_state_m128i s;
    s.A = _mm_cvtsi32_si128((int)state[0]);
    s.B = _mm_cvtsi32_si128((int)state[1]);
    s.C = _mm_cvtsi32_si128((int)state[2]);
    s.D = _mm_cvtsi32_si128((int)state[3]);
    md5_block_avx512(&s, block);
    state[0] = (uint32_t)_mm_cvtsi128_si32(s.A);
    state[1] = (uint32_t)_mm_cvtsi128_si32(s.B);
    state[2] = (uint32_t)_mm_cvtsi128_si32(s.C);
    state[3] = (uint32_t)_mm_cvtsi128_si32(s.D);
}
